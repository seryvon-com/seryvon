...
_REASON_EN: dict[str, str] = {
    ...
    "opr_not_configured": "Domain authority not configured — add a DataForSEO key (recommended) or an OPR key.",
    "opr_no_data": "DataForSEO active but returned no authority data — domain may not be indexed yet in DataForSEO's databases.",
    "no_backlink_source": "No referring-domains source configured — add a DataForSEO key to enable domain rank (authority.opr). Referring-domain count requires the DataForSEO Backlinks API ($100 minimum deposit).",
    "no_backlink_data": "DataForSEO Backlinks API requires a separate $100 minimum deposit to activate access — referring-domain data is not available on the standard plan.",
    "cwv_metric_unavailable": "{metric} unavailable — PSI key not configured.",
    "cwv_no_crux_data": "{metric} unavailable — PSI active but no CrUX field data (site traffic too low; ~1 000 visits/28 days required).",
...
_REASON_FR: dict[str, str] = {
    ...
    "opr_not_configured": "Autorité de domaine non configurée — ajouter une clé DataForSEO (recommandé) ou OPR.",
    "opr_no_data": "DataForSEO actif mais aucune donnée d'autorité retournée — le domaine n'est peut-être pas encore indexé dans les bases DataForSEO.",
    "no_backlink_source": "Aucune source de domaines référents configurée — la clé DataForSEO donne le score d'autorité (authority.opr). Les domaines référents nécessitent l'API Backlinks DataForSEO (dépôt minimum 100 $).",
    "no_backlink_data": "L'API Backlinks DataForSEO nécessite un dépôt minimum de 100 $ pour être activée — les données de domaines référents ne sont pas disponibles sur l'offre standard.",
    "cwv_metric_unavailable": "{metric} indisponible — clé PSI non configurée.",
    "cwv_no_crux_data": "{metric} indisponible — PSI actif mais pas de données CrUX terrain (trafic trop faible ; ~1 000 visites/28 jours requises).",
...
_EXPL_EN: dict[str, str] = {
    ...
    "authority_opr": "Domain authority (DataForSEO backlink rank, normalised 0–10): {opr}/10.",
...
_EXPL_FR: dict[str, str] = {
    ...
    "authority_opr": "Autorité de domaine (backlink rank DataForSEO normalisé 0–10) : {opr}/10.",

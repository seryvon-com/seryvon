...
from seryvon.i18n import get_locale
...
<header>
  ...
  <p class="meta">
    Seryvon {{ report.tool_version }} · {{ generated }} · {{ labels.schema }}
    v{{ report.schema_version }}
    · {{ labels.coverage }} {{ (report.coverage * 100)|round|int }}%
...
<section class="pillars">
  ...
  <div class="pillar {{ p.css }}">
    ...
    <div class="pmeta">{{ p.measured }} {{ labels.measured }} ·
      {{ p.excluded }} {{ labels.excluded }} ·
      {{ (p.coverage * 100)|round|int }}% {{ labels.coverage_short }}</div>
  </div>
  {% endfor %}
</section>
{% if report.issues %}
<section class="criteria">
  <h2>{{ labels.action_plan }} — {{ report.issues|length }} {{ labels.problems }}</h2>
  <table class="criteria-table">
    <colgroup><col class="priority"><col class="criterion"><col class="severity">
      <col class="impact"><col class="effort"><col class="recommendation"></colgroup>
    <thead><tr>
      <th>{{ labels.priority }}</th><th>{{ labels.criterion }}</th><th>{{ labels.severity }}</th>
      <th>{{ labels.impact }}</th><th>{{ labels.effort }}</th><th>{{ labels.recommendation }}</th>
    </tr></thead>
    <tbody>
...
<section class="criteria">
  ...
  <table class="criteria-table">
    <colgroup><col class="criterion"><col class="status"><col class="score"><col class="weight">
      <col class="explanation"><col class="threshold"><col class="measured">
      <col class="source"></colgroup>
    <thead><tr>
      <th>{{ labels.criterion }}</th><th>{{ labels.status }}</th>
      <th>{{ labels.score }}</th><th>{{ labels.weight }}</th>
      <th>{{ labels.explanation }}</th><th>{{ labels.threshold }}</th>
      <th>{{ labels.measured_value }}</th><th>{{ labels.source }}</th>
    </tr></thead>
    <tbody>
    {% for r in s.rows %}
      <tr>
...
{% endfor %}
<footer>{{ labels.generated }} Seryvon — {{ labels.deterministic }} SEO · GEO · GSO ·
  AEO · ASO.</footer>
</body>
</html>
"""
    ...
    labels = (
        {
            ...
            "deterministic": "deterministic audit",
        }
        if locale == "en"
        else {
            "schema": "schéma",
            ...
            "source": "Source",
            "generated": "Généré par",
            "deterministic": "audit déterministe",
        }
    )
